    ...
    for mat in s:
       ...
       # print(tag_data)
        for key, value in tag_data.iteritems():
            shader = value["shader"]
            if shader is None:
                print("WARN: No shader attached to material.")
                continue
            elif not "filename" in shader: #Doesnt find files. Why??
                print("INFO: Material has no textures")
                continue
            ...
            shader["filename"] = path.join("textures", real_filename)
...
def main():
    ...
    # serialize materials
    mats = serialize_materials(materials, doc)
